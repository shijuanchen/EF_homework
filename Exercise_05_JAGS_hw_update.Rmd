...
data = list(
  N = 297, ## sample size
  mu0=20,  ## prior mean 
  T=0.01,  ## data precision
  S = 1/27, ## prior precision
...
```{r}
#Run three MCMC chains. Give one initial condition list for each chain. Number of interation is 1000.
inits = list()
inits[[1]] = list(mu=10)
inits[[2]] = list(mu=15)
inits[[3]] = list(mu=20)
...
jags.out   <- coda.samples (model = j.model,
                            variable.names = c("mu"),
                                n.iter = 1000)
plot(jags.out) # plot the traceplot and density plot. 
```
...
```
```{r}
GBR <- gelman.plot(jags.out) # plot the GBR statistics
```

```{r}
burnin = 500 # remove the first 500 steps
jags.burn <- window(jags.out,start=burnin)  ## remove burn-in
plot(jags.burn)                             ## check diagnostics post burn-in
```
```{r}
...
model {
  mu ~ dnorm(mu0,T) # prior on the mean 
  S ~ dnorm(S0, T) # prior on the precision 
  for(i in 1:N){
    X[i] ~ dnorm(mu,S) # data model
  }
}
...
data = list(
  ...
  mu0=20,  ## prior mean 
  T=0.01,  ## data precision
  S0 = 1/27, ## prior precision
  X = c(20.9, 13.6, 15.7, 6.3, 2.7, 25.6, 4, 20.9, 7.8, 27.1, 25.2, 19, 17.8, 22.8, 12.5, 21.1, 22, 22.4, 5.1, 16, 20.7, 15.7, 5.5, 18.9, 22.9, 15.5, 18.6, 19.3, 14.2, 12.3, 11.8, 26.8, 17, 5.7, 12, 19.8, 19, 23.6, 19.9, 8.4, 22, 18.1, 21.6, 17, 12.4, 2.9, 22.6, 20.8, 18.2, 14.2, 17.3, 14.5, 8.6, 9.1, 2.6, 19.8, 20, 22.2, 10.2, 12.9, 20.9, 21.1, 7.3, 5.8, 23.1, 17, 21.5, 10.1, 18.4, 22.6, 21.2, 21.5, 22.4, 17.3, 16, 25, 22.4, 23.9, 23, 21.9, 19, 28.6, 16, 22.5, 23.2, 8.7, 23.4, 15.3, 25.6, 19.2, 17.4, 23.8, 20.4, 19, 3.6, 23.4, 19.6, 17.5, 16.5, 22, 19.7, 7.35, 18, 17.8, 9.6, 15, 12, 17.7, 21.4, 17, 22.1, 18.9, 15.05, 12.9, 19.3, 15.3, 13.6, 15.4, 10.6, 11.3, 11.8, 22.2, 22.2, 13.1, 7.4, 4.5, 11.7, 19.5, 19.9, 11.6, 13.9, 15.5, 11, 18.6, 17.6, 12.7, 20.9, 18.8, 22.4, 21.2, 18.2, 15.3, 13.6, 7.3, 17.4, 17.4, 10.5, 22.9, 23.2, 13.8, 14.8, 22.2, 20.9, 13, 18.9, 19, 15.2, 16.8, 18, 24.6, 15.4, 17.2, 23.2, 22.8, 25.5, 7.8, 6, 6.4, 19, 13.5, 23.7, 18, 22.2, 22.4, 9.3, 13.7, 18.9, 20.5, 23.3, 20.8, 18.4, 4.5, 12.2, 16.9, 13.5, 17.8, 16.9, 20.4, 19.5, 22.2, 24.5, 21.2, 16.5, 18, 16.4, 3.9, 17.9, 22, 12.9, 21, 18, 9.2, 15.9, 8.1, 8.3, 10.7, 12, 19.9, 13.6, 17.3, 11.5, 12.4, 15.1, 22, 19.3, 17.5, 14.5, 14.7, 17.5, 19.6, 12.9, 20.3, 17.9, 20.2, 18.3, 9.5, 19, 21, 13.1, 20.4, 16.3, 18.3, 11.8, 23.3, 15.2, 20, 17.9, 12, 19.6, 18.5, 16.2, 10.9, 17.8, 13.8, 10, 17.9, 15.6, 20.3, 14.9, 18.6, 12.5, 18.2, 16, 18.7, 18, 15.3, 19, 17.9, 15.8, 17.7, 14.4, 19.6, 18.3, 18.7, 17.8, 18, 10.1, 18.8, 16.4, 21.2, 16.6, 16.7, 17.8, 16.5, 19.3, 16.3, 14.2, 13, 9.4, 19.7, 13.4, 2.6, 17.6, 16.7, 17.6, 5.8, 17.6, 20.1, 18.2, 16.7, 14, 13.9, 5.1, 16.6, 3.9, 17.5, 18)
  )
```

```{r}
#Run three MCMC chains. Give one initial condition list for each chain. Number of interation is 10000.
inits = list()
inits[[1]] = list(mu=10, S=0.005)
inits[[2]] = list(mu=15, S=0.01)
inits[[3]] = list(mu=20, S=0.02)
j.model   <- jags.model (file = textConnection(NormalN_S),
                             data = data,
                             inits = inits,
                             n.chains = 3)
jags.out2   <- coda.samples (model = j.model,
                            variable.names = c("mu","S"),
                                n.iter = 2000)
plot(jags.out2) # plot the traceplot and density plot of mean. 
```
```{r}
...
```
```{r}
burnin2 = 1000                                ## determine convergence
...
The prior of precision is chose as normal distribution with mean of S0, and prior percision of T. 

Describe the changes in the distribution of the mean (shape, location, tails) compared to Task 1:

The shape of the distribution of the mean is very similar, but they also have some differences. The distribution of the mean in Task 2 is more symmetric than that of Task 1. The mean of the distribution of mu (mean) of task 1 is 16.571, whereas the mean of the distribution of mu (mean) of task 2 is 16.578. 

Feedback:
Moving forward, I’d recommend increasing the length of your MCMC to ensure that the effective sample size is at least 5000
There is no need to thin a MCMC unless the output are so large that you’re running into trouble storing them  [-0.1]
You shouldn't use a Normal prior on a precision because precisions, by definition, can’t be negative, but the Normal definitely can generate negative values [-0.25]
Choice of prior parameter values is poorly justified. Specifically, you’re using the 1/27 value as the mean without knowing where it came from or whether you think it makes sense. Similarly, the choice of prior precision was based on mimicry, not thinking about whether that value was appropriate for the situation. [-0.25] 
