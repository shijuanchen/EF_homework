...
```{r, echo=TRUE}
   ...
   log(1/5)      ## f. ln(1/5)
```
g. If the answer is infinity, the output is "Inf". If the expression has no meaning, the output is NaNs (not a number) and R would give a warning saying that NaNs produced.

2. The ouput of atan and atan2 are in the unit of radians. If we want the output to be represented in degrees, we need to multiply the outputs by 180/pi.

3. The atan function only takes one variable (a number), whereas the atan2 function takes two variables (the x, y values of a vector). The atan2 function returns the angle between the x-axis and the vector from the origin to (x, y).

4. In R, log is the natural logarithm, whereas log10 is logarithm with base 10.
...
```{r, echo=TRUE}
   start_density = 5
   rate = 0.04
   population = 5 * (1+0.04)^pi
   print(population)  # population size in π years
```
...
### Answers:
8. The output of sum is a number that sums up all values in a vector, whereas cumsum is a vector in which each number is the sum of the values from the first value to the value in the orginal vector at the same position.
...
```{r, echo=TRUE}
   ...
   print(x)    # the sequence
   diff(x)[1]  # the differences
   sum(exp(x)) # The sum of the exponential of this sequence
```
...
```{r, echo=TRUE}
   ...
   diff_1 = diff(x)
   diff_2 = diff(diff_1)
   print(diff_2)  # The second difference
```
