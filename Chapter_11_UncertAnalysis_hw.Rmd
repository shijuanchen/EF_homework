...
The initial condition uncertainty is low, since when changing the initial condition uncertainty, the prediction does not change much. Also, the initial uncertainty decreases as time goes, since the effect of initial condition of the prediction is larger in the ealier than the later time (the initial condition is more important in ealier time), which meets my expectations. 
...
A) In the forecasting period, the parameter uncertainty is stable as time goes, and relative magnitude of the parameter uncertainty does not change much as time goes. Also, the parameter uncertainty is larger than the initial condition uncertainty. This aligns with my expectation, because the parameter uncertainty is determined by how well we understand the process in the system, and does not increase or decrease as time goes. 
B) To partition the contribution of the parameters, r, Kg, alpha, beta, I will calculate the variance contributed by each paramters, and the total variance of the estimates, and calculate the percentage of each parameter contributes to the total variance. For example, I can consider uncertainty of one parameter each time, and keep other parameter constant, to calculate the uncertainty for each parameters.
...
The driver of the uncertain increases as time goes. At first, the driver uncertainty almost has the same magnitude as the parameter uncertainty, the magnitude of the driver uncertainty increases and always larger than the parameter uncertainty. This meet our expectation, since the predictability of drivers decreases as time goes. 
...
The relative contribution of the process error decline with time, since the driver uncertainty increase over time and contributes more and more as time goes. The process error uncertainty does not change much, but since the driver uncertainty increases, the relative contribution of process error decline with time.
...
In the out-of_sample preditions, the random effects contributes the largest proportion of variance, so the research efforts would be focused on how to narrow down the random effects variance. To narrow down the random effects variance, it might be helpful to add other meaningful effects (variables) to the model. Also, the process uncertainty and driver uncertainty increase significantly at around time 55, so it might be worthwhile to understand the change at time 55. In the in-sample prediction, we should study the uncertainty of process error and drive error. For example, we may consider using a more complex model. 
...
```{r}
plot.run()
ecoforecastR::ciEnvelope(time2,N.IPDEA.mean-1.96 * N.IPDEA.sd,N.IPDEA.mean+1.96 * N.IPDEA.sd,col=col.alpha(N.cols[5],trans))
ecoforecastR::ciEnvelope(time2,N.IPDE.mean-1.96 * N.IPDE.sd,N.IPDE.mean+1.96 * N.IPDE.sd,col=col.alpha(N.cols[4],trans))
ecoforecastR::ciEnvelope(time2,N.IPD.mean-1.96 * N.IPD.sd,N.IPD.mean+1.96 * N.IPD.sd,col=col.alpha(N.cols[3],trans))
ecoforecastR::ciEnvelope(time2,N.IP.mean-1.96 * N.IP.sd,N.IP.mean+1.96 * N.IP.sd,col=col.alpha(N.cols[2],trans))
ecoforecastR::ciEnvelope(time2,N.I.mean-1.96 * N.I.sd,N.I.mean+1.96 * N.I.sd,col=col.alpha(N.cols[1],trans))
...
trans1 = 1
trans2 = 0.3
ecoforecastR::ciEnvelope(time2,N.IPDEA.mean-1.96 * N.IPDEA.sd,N.IPDEA.mean+1.96 * N.IPDEA.sd,col=col.alpha(N.cols[5],trans1))
ecoforecastR::ciEnvelope(time2,N.IPDEA.ci[1,],N.IPDEA.ci[3,],col=col.alpha(N.cols[5],trans2))
lines(time2,N.I.mean,lwd=0.5)
plot.run()
ecoforecastR::ciEnvelope(time2,N.IPDE.mean-1.96 * N.IPDE.sd,N.IPDE.mean+1.96 * N.IPDE.sd,col=col.alpha(N.cols[4],trans1))
ecoforecastR::ciEnvelope(time2,N.IPDE.ci[1,],N.IPDE.ci[3,],col=col.alpha(N.cols[4],trans2))
lines(time2,N.I.mean,lwd=0.5)
plot.run()
ecoforecastR::ciEnvelope(time2,N.IPD.mean-1.96 * N.IPD.sd,N.IPD.mean+1.96 * N.IPD.sd,col=col.alpha(N.cols[3],trans1))
ecoforecastR::ciEnvelope(time2,N.IPD.ci[1,],N.IPD.ci[3,],col=col.alpha(N.cols[3],trans2))
lines(time2,N.I.mean,lwd=0.5)
plot.run()
ecoforecastR::ciEnvelope(time2,N.IP.mean-1.96 * N.IP.sd,N.IP.mean+1.96 * N.IP.sd,col=col.alpha(N.cols[2],trans1))
ecoforecastR::ciEnvelope(time2,N.IP.ci[1,],N.IP.ci[3,],col=col.alpha(N.cols[2],trans2))
lines(time2,N.I.mean,lwd=0.5)
plot.run()
ecoforecastR::ciEnvelope(time2,N.I.mean-1.96 * N.I.sd,N.I.mean+1.96 * N.I.sd,col=col.alpha(N.cols[1],trans1))
ecoforecastR::ciEnvelope(time2,N.I.ci[1,],N.I.ci[3,],col=col.alpha(N.cols[1],trans2))
lines(time2,N.I.mean,lwd=0.5)
```

The figures shows the comparison of each part of the uncertainty of the Gaussian approximation (transparency =1) and the full Monte Carlo ensemble (transparency=0.3). The Gaussian approximation and the full Monte Carlo ensemble are very similar.
...
reduce the sample size `Nmc` by 20x, so the Nmc now is set to 50.
...
The first figure is the Monte Carlo estimates. The second figure is the Gaussian approximation. The Monte Carlo estimates after reducing sample size is very different from the original one. The overall uncertainty is larger. The Gaussian approximation is also different from the original one, but the difference is smaller. Overall, when the sample size is large, Monte Carlo estimates and Gaussian approximation produce similiar results, whereas with small sample size, Monte Carlo estimates and Gaussian approximation have large differences. Also, Gaussian approximation seems to produce better results when the sample size is small.
