...
```
```{r}
burnin = 500 # determine the burn in as 500 steps.
var.burn <- window(var.out,start=burnin) # remove burnin
...
The autocorrelation is very low, which is good.

```{r}
effectiveSize(var.burn)
```
The effective sample size is under 5000, so increase the sample size.
```{r}
var.out2   <- coda.samples (model = j.model,
                            variable.names = c("beta","prec"),
                                n.iter = 5000)
```
```{r}
gelman.diag(var.out2)
```
```{r}
GBR <- gelman.plot(var.out2)
```
```{r}
burnin2 = 3000 # determine the burn in steps.
var.burn2 <- window(var.out2,start=burnin2) # remove burnin
plot(var.burn2)  # ## check diagnostics post burn-in
```
```{r}
effectiveSize(var.burn2)
```
```{r}
acfplot(var.burn2)
```

Now, we have enough sample size and the autocorrelation is very low.
...
```{r}
## convert to matrix
var.mat2      <- as.matrix(var.burn2)
plot_hist_function_1 <- function(beta, bx){
  beta_x = var.mat2[,c(beta)]
  hist(beta_x, breaks = 100, main=beta,xlab=beta)
  abline(v=bx, col=2, lwd=3)
  abline(v=quantile(beta_x, c(0.025, 0.25, 0.50, 0.75, 0.975)), lty="dotted", col=3, lwd=3)
}

plot_hist_function_1("beta[1]", b0)
plot_hist_function_1("beta[2]", b1)
plot_hist_function_1("prec", 1/(sigma*sigma))

```

The green dotted lines indicate 2.5%, 25%, 50%, 75%, 97.5% quantiles of the estimates. The red line indicate the "true" parameters.
...
```
```{r}
cor_2 = cor(var.mat2)    ## correlation matrix among model parameters
print(cor_2)
```

In the pairs plot, beta[1] (intercept) and beta[2] (slope) have a strong negative correlation, `r cor_2[1,2]`. The correlation between beta[1] (intercept) and precision is `r cor_2[1,3]`, which is a very weak correlation. The correlation between beta[2] (slope) and precision is `r cor_2[2,3]`, which is a very weak correlation. In summary, the intercept and slope are negatively correlated, but both of them are amost independent with precision.
...
Build the classical linear regression model:
```{r}
linearModel <- lm(y ~ x1)
summary(linearModel)  # model summary
linear_coef <- summary(linearModel)$coefficients
linear_st <- summary(linearModel)$sigma
```
The summary statistics for the Bayesian regression model is 
```{r}
summary(var.burn2)
```

Comparison: For classic linear regression, the estimated intercept is `r linear_coef[1,1]`, and the estimated slope is `r linear_coef[2, 1]`. For Bayesian regression model, the estimated intercept is `r mean(var.mat2[,c("beta[1]")])`, and the estimated slope is `r mean(var.mat2[,c("beta[2]")])`.  The standard error is `r linear_st` of the classical linear regression. Since the precision is 1/variance, so the precision of the classical linear regression is `r 1.0/(linear_st*linear_st)`. The estimated precision of Bayesian regression model is `r mean(var.mat2[,c("prec")])`. Using these two different approaches, the estimates are very similiar, but Bayesian regression model characterize the probability distribution of the estimates and uncertainty better than the classic regression model.
...
Based on the histograms for the estimated model parameters, the "true" parameter always fall into (2.5%, 97.5%) quantiles of the estimates of the model parameters. Also, the uncertainty of the model can be well evaluated by calculating the credible interval around the regression line. Therefore, the the statistical analysis recovers the true model well.
...
data2 <- list(x1=x1, x2=x2, y=y, n=n)

multivariate_regression <- "
model{
  beta ~ dmnorm(b0, Vb)  # prior on vector of regression params
  prec ~ dgamma(s1,s2)
  for(i in 1:n){
      mu[i] <- beta[1] + beta[2]*x1[i] + beta[3]*x2[i] + beta[4]*x1[i]*x2[i]  ## process model
      y[i]  ~ dnorm(mu[i],prec)             ## data model
  }
}
"
...
inits2 <- list()
for(i in 1:nchain){
 inits2[[i]] <- list(beta = rnorm(4,0,5), prec = runif(1,1/100,1/20))
}

j.model_mv   <- jags.model(file = textConnection(multivariate_regression),
                             data = data2,
                             inits = inits2,
                             n.chains = nchain)
...
```{r}
var.out_mv   <- coda.samples (model = j.model_mv,
                            variable.names = c("beta","prec"),
                                n.iter = 5000)
## assess convergence and remove burn-in before doing other diagnostics
GBR <- gelman.plot(var.out_mv)
gelman.diag(var.out_mv)
```
```{r}
burnin = 1000 # determine the burn in as 1000 steps.
var.burn_mv <- window(var.out_mv,start=burnin) # remove burnin
plot(var.burn_mv)  # ## check diagnostics post burn-in
```
```{r}
effectiveSize(var.burn_mv)
```
Effective sample size is larger than 5000. Good.
...
```{r}
var.mat_mv      <- as.matrix(var.out_mv)
pairs(var.mat_mv)  ## pairs plot to evaluate parameter correlation
```
```{r}
cor(var.mat_mv)
```
```{r}
plot_hist_function <- function(beta, bx){
  beta_x = var.mat_mv[,c(beta)]
  hist(beta_x, breaks = 100, main=beta,xlab=beta)
  abline(v=bx, col=2, lwd=3)
  abline(v=quantile(beta_x, c(0.025, 0.25, 0.50, 0.75, 0.975)), lty="dotted", col=3, lwd=3)
}
plot_hist_function("beta[1]", b0)
plot_hist_function("beta[2]", b1)
plot_hist_function("beta[3]", b2)
plot_hist_function("beta[4]", b3)
plot_hist_function("prec", 1/(sigma*sigma))
```
...
The "true" parameters always fall into (2.5%, 97.5%) quantiles of the estimates. The statistical analysis recover the true model well. The model uncertainty can also be evalutated using credible and prediction intervals (I don't know how to plot it exactly). 
```{r}
# ## credible and prediction intervals
# nsamp <- 5000
# samp <- sample.int(nrow(var.mat_mv),nsamp)
# xpred <- 0:20                   ## sequence of x values we're going to
# npred <- length(xpred)              ##      make predictions for
# ypred <- matrix(0.0,nrow=nsamp,ncol=npred)  ## storage for predictive interval
# ycred <- matrix(0.0,nrow=nsamp,ncol=npred)  ## storage for credible interval
# for(g in seq_len(nsamp)){
#   theta = var.mat_mv[samp[g],]
#   ycred[g,] <- theta["beta[1]"] + theta["beta[2]"]*xpred + theta["beta[2]"]*7.5 + theta["beta[3]"]*xpred*7.5
#   ypred[g,] <- rnorm(npred,ycred[g,],1/sqrt(theta["prec"]))
# }
# ci <- apply(ycred,2,quantile,c(0.025,0.5,0.975))  ## credible interval and median
# pi <- apply(ypred,2,quantile,c(0.025,0.975))        ## prediction interval
