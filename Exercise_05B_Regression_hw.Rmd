...
acfplot(var.burn)
```
autocorrelation is very low, which is good.
```{r}
...
plot_hist_function_1 <- function(beta, bx){
  ...
  abline(v=quantile(beta_x, c(0.25, 0.50, 0.75)), lty="dotted", col=3, lwd=3)
}
...
The green dotted lines indicate 0.25, 0.50, 0.75 percentiles. The red line indicate the "true" parameters.
...
In the pairs plot, beta[1] (intercept) and beta[2] (slope) have a strong negative correlation, `r cor_2[1,2]`. The correlation between beta[1] (intercept) and precision is `r cor_2[1,3]`, which is a very weak negative correlation. The correlation between beta[2] (slope) and precision is `r cor_2[2,3]`, which is a very weak positive correlation. In summary, the intercept and slope are negatively correlated, but both of them are amost independent with precision.
...
The fit parameters are close to the "true" parameters we used to generate the pseudo-data. The statistical analysis recovers the true model well in some degree, though it could be improved if we have more data.
...
The green dotted lines show the 0.25, 0.50, 0.75 percentiles of estimates, the red solid line show the "true" parameters.
