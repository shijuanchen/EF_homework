...
```{r}
beta1 = var.mat[,c("beta[1]")]
hist(beta1, breaks = 100)
beta2 = var.mat[,c("beta[2]")]
hist(beta2, breaks = 100)
precision = var.mat[,c("prec")]
...
```
```{r}
cor(var.mat2)    ## correlation matrix among model parameters
```
In the pairs plot, beta[1] (intercept) and beta[2] (slope) have a strong negative correlation, -0.887. The correlation between beta[1] (intercept) and precision is about -0.0102, which is a very weak negative correlation. The correlation between beta[2] (slope) and precision is about 0.0126, which is a very weak positive correlation. In summary, the intercept and slope are negatively correlated, but both of them are amost independent with precision.
...
Comparison: For classic linear regression, the estimated intercept is 10.542, and the estimated slope is 1.987. For Bayesian regression model, the estimated intercept is 10.535, and the estimated slope is 1.987.  The standard error is 3.806 of the classical linear regression. Since the precision is 1/variance, so the precision of the classical linear regression is 0.069. The estimated precision of Bayesian regression model is 0.051. Using these two different approaches, the estimates are very similiar, but Bayesian regression model characterize the probability distribution of the estimates and uncertainty better than the classic regression model.
