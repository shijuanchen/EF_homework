...
#combined <- matchInventoryRings(trees,rings,nyears=15) original setting
combined <- matchInventoryRings(trees,rings,nyears=30)
...
Run the model with random effects off; increase the number of years to 30 and MCMC to 50000
...
for(i in 1:ni){
  ...
  burnin = 30000
  jags.burn <- window(jags.out,start=burnin) # remove burnin
  gelman.diag(jags.burn)
  plot(jags.burn)
  ## run MCMC
  jags.out.x   <- coda.samples (model = j.model,
                              variable.names = c("x","tau_add","tau_dbh","tau_inc","mu"),
                              n.iter = n.iter)
```
...
## state variable initial condition (subtract observed diameter increments off from the observed diameter)
  ...
  ## compile JAGS model
  j.model   <- jags.model (file = textConnection(TreeDataFusionMV_rf),
                           data = data,
                           inits = init,
                           n.chains = 3)
  ...
  burnin = 30000
  jags.burn2 <- window(jags.out2,start=burnin) # remove burnin
  gelman.diag(jags.burn2)
  plot(jags.burn2)
  
```

What is the relative partitioning of uncertainties in the different versions of the model among observation error, process error, and the different random effects? What does the size of these effects suggest about the drivers of uncertainty in tree growth?

```{r}
var.out1 = as.matrix(jags.out)
var_add1 = mean(1/var.out1[2,])   
var_dbh1 = mean(1/var.out1[3,])
var_inc1 = mean(1/var.out1[4,])
total1 = var_add1 + var_dbh1 + var_inc1
per_add1 = (var_add1 / total1) * 100  # process error of growth
per_dbh1 = (var_dbh1 / total1) * 100  # observation error of DBH
per_inc1 = (var_inc1 / total1) * 100  # observation error of tree rings
var.out2 = as.matrix(jags.out2)
var_add2 = mean(1/var.out2[2,])
var_dbh2 = mean(1/var.out2[3,])
var_inc2 = mean(1/var.out2[4,])
var_ind2 = mean(1/var.out2[5,])
var_yr2 = mean(1/var.out2[6,])
total2 = var_add2 + var_dbh2 + var_inc2 + var_ind2 + var_yr2
per_add2 = (var_add2 / total2) * 100  # process error of growth
per_dbh2 = (var_dbh2 / total2) * 100  # observation error of DBH
per_inc2 = (var_inc2 / total2) * 100  # observation error of tree rings
per_ind2 = (var_ind2 / total2) * 100  # random effects of individuals
per_yr2 = (var_yr2 / total2) * 100    # random effect of year
```

For the first version of the model (without random effects), the variance of the process error of growth, the observation error of DBH, and the observation error of tree rings is `r var_add1`, `r var_dbh1`,and `r var_inc1`, respectively. In the first model, the percentage of variance of the process error of growth, the observation error of DBH, and the observation error of tree rings is `r per_add1`%, `r per_dbh1`%,and `r per_inc1`%, respectively. 

For the second version of the model (with random effects), the variance of the process error of growth, the observation error of DBH, the observation error of tree rings, the random effects of individuals, and random effect of year is `r var_add2`, `r var_dbh2`, `r var_inc2`, `r var_ind2`, and `r var_yr2`, respectively. In the second model, the percentage of variance of the process error of growth, the observation error of DBH, the observation error of tree rings, the random effects of individuals, and random effect of year  is `r per_add2`%, `r per_dbh2`%, `r per_inc2`%, `r var_ind2`%, and `r var_yr2`%, respectively. 

In the two version of the model, the process error uncertainty is the largest. In the second model, the random effects error uncertainty is very small.
...
I propose to add annual temperature as a fixed effect to the model. The assumption is that annual temperature is positively correlated with tree ring width. 
...
I will make the time series plot of the annual temperature versus the year, and also the time series plot of the tree ring data. I will also calculate the correlation to see if there is any relationship between the two. 

5. Write the JAGS code that would fit the proposed model (note: you don't have to run this model, just propose the code)

Suppose to add "temp"" as annual temperature variable. The priors of temperature can be read from the external data sources.

The jags model would be like this:
